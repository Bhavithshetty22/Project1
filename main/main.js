...
let activeIndex = -1;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleDayActive(dayElement) {
                dayElement.classList.toggle('active');
                if (dayElement.classList.contains('active')) {
                    dayElement.innerHTML = '<i class="fas fa-bolt"></i>';
                } else {
                    dayElement.textContent = dayElement.getAttribute('data-day') || 'F';
                }
            }
        
            // Optional: Add click handlers for days
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function generateGradient(ctx, colorStart, colorEnd) {
                let gradient = ctx.createLinearGradient(0, 0, 0, 350); // Vertical gradient
                gradient.addColorStop(0.2, colorStart); // Near the line, less opaque
                gradient.addColorStop(0.8, colorEnd); // Away from the line, more opaque
                return gradient;
            }
        
            // Create gradients for datasets
            const pomodoroGradient = generateGradient(studyCtx, 'rgba(255, 201, 60, 0.3)', 'rgba(255, 201, 60, 0.1)');
            const todoGradient = generateGradient(studyCtx, 'rgba(255, 111, 60, 0.3)', 'rgba(255, 111, 60, 0.1)');
        
            // Create the study analytics chart
            new Chart(studyCtx, {
                type: 'line',
                data: {
                    labels: ['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'],
                    datasets: [
                        {
                            ...
                            data: [5.7, 4.5, 4, 6.5, 6.2, 9, 6.3],
                            borderColor: '#ffc93c',
                            backgroundColor: pomodoroGradient,
                            borderWidth: 3,
                            fill: true,
                            tension: 0.4,
                            pointRadius: 0,
                    ...
                    plugins: {
                        legend: {
                            ...
                            onClick: (event, legendItem, legend) => {
                                const studyChart = legend.chart;
                                const datasetList = studyChart.data.datasets;
                                const datasetIndex = legendItem.datasetIndex;
                                const isSingleVisible = datasetList.every((dataset, i) => i === datasetIndex || dataset.hidden);
                
                                datasetList.forEach((dataset, i) => {
                                    dataset.hidden = isSingleVisible ? false : i !== datasetIndex;
                                });
